...
export class SalesforceClient {
    ...
    async emailReturnLabel(returnOrderId, customerEmail) {
        try {
            // Input validation
      ...
      `;
            ...
            const returnOrder = result.records[0];
    ...
    async createCaseFromReturn(returnOrderId) {
      ...
      `;
            ...
            const returnOrder = returnResult.records[0];
      ...
      `;
            ...
            const hasDefectiveItems = returnOrder.ReturnOrderLineItems?.records?.some((item) => ['Defective', 'Damaged', 'Quality Issue'].includes(item.ReasonCode));
            ...
            if (this.config.slackWebhookUrl) {
                const lineItemSummary = returnOrder.ReturnOrderLineItems?.records?.map((item) => `${item.Product2?.Name || 'Unknown'} (${item.Quantity})`).join(', ') || 'No items';
                await this.sendSlackAlert({
    ...
    getSlackIconByPriority(priority) {
        switch (priority) {
    ...
    getSlackColorByPriority(priority) {
        switch (priority) {
